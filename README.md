...
상단 탭의 **20일 신고가·신저가**에서는 유니버스 전체(코스피200+코스닥150+S&P500+ETF) 중
오늘 종가가 최근 20거래일(약 1개월) 동안의 최고가/최저가와 같은 종목을 시장별로 필터링해서
...
### 7. (선택) 20일 신고가·신저가 화면 첫 실행

`https://<아이디>.github.io/rs-dashboard/w52.html` — 아래 "별도 화면" 참고. **Actions →
20일 신고가·신저가 갱신 → Run workflow** 를 한 번 실행해야 `data/w52.json`이 생기고
샘플 대신 실제 데이터가 뜹니다.

## 별도 화면 — 20일 신고가·신저가 스크리너 (`w52.html`)
...
- **계산**: RS(장기/단기 상대강도)는 계산하지 않고, 오직 **종가 기준** 최근 20거래일(약 1개월,
  원래 252거래일=52주로 했다가 너무 길다는 판단으로 줄임) 최고가/최저가만 봅니다. 오늘 종가가
  그 최고가/최저가와 같으면 신고가/신저가로 표시합니다.
- **시장별로 하루 최대 상위 20개만 기록**: 신고가/신저가 종목이 많은 날엔 시장(코스피/코스닥/
  한국ETF/S&P500/미국ETF)별로 따로 나눠서, 그 안에서 "오늘을 뺀 20일 구간의 이전 최고가/
  최저가 대비 오늘 종가가 몇 % 위/아래인지"(갱신폭)가 큰 순으로 시장당 `--top-n`(기본 20)개씩만
  남깁니다(전체를 한 데 모아 자르면 코스피·코스닥처럼 종목 수가 많은 시장이 결과를 독차지해서
  미국 ETF 같은 작은 시장은 하나도 안 뽑히는 문제가 있었습니다). 개수는 `rs/compute_52w.py`의
  `W52_TOP_N` 환경변수 또는 워크플로에서 `--top-n` 인자로 바꿀 수 있습니다.
- **사유(자동 검색, 추정)**: 추려진 종목마다 그 종목 이름으로 자동 검색한 최신 관련 기사 1건을
  "사유" 칸에 보여줍니다 — **Google 뉴스 RSS**(한국은 한국어, 미국은 영어)를 1차로 쓰고,
  한국 종목이 그걸로 못 찾으면 네이버 금융 뉴스 페이지도 보조로 한 번 더 시도합니다(처음엔
  네이버만 썼는데 한국 종목이 전부 "관련 뉴스 없음"으로 나오는 문제가 있어서, 이미 잘 동작하던
  Google 뉴스 RSS 방식을 한국에도 1차로 적용했습니다). **사람이 실제 원인을 확인/검증한 게
  아니라 추정**이라 관련 없는 기사가 걸리거나 못 찾을 수 있습니다(그 종목 이름으로 뜨는 최신
  기사일 뿐). 공시(DART) 연동은 종목코드 → DART corp_code 매핑이 추가로 필요해서 아직 넣지
  않았습니다 — 원하면 나중에 추가할 수 있습니다. `--no-reason` 옵션으로 이 조회를 생략할 수
  있습니다(뉴스 검색 실패해도 전체 실행은 절대 실패하지 않습니다 — 그 종목만 "관련 뉴스 없음"
  으로 표시). 실행 로그의 `[w52] 관련 기사(사유) 자동 검색: N/M건 발견 (소스별 {...})`에서
  어느 소스가 몇 건이나 찾았는지 확인할 수 있습니다 — 특정 소스가 계속 0건이면 그 소스가
  막혔거나 페이지 구조가 바뀐 것입니다.
- **데이터 크기 절약**: 스캔한 전 종목을 다 담지 않고, 시장별 상위 20개(신고가)+상위 20개
  (신저가)만 `data/w52.json`에 저장합니다. `totalHigh52`/`totalLow52`(자르기 전 전체 건수)도
  같이 저장해서
  화면에 "6 / 전체 34종목"처럼 보여줍니다.
...
- **20일 신고가·신저가 갱신 주기/동시성**: `.github/workflows/w52.yml` 의 `cron`(기본 하루 한 번),
  `rs/compute_52w.py` 의 `W52_MAX_WORKERS` 환경변수(기본 24).
- **20일 신고가·신저가 하루 최대 종목 수**: `W52_TOP_N` 환경변수 또는 `--top-n` 인자(기본 20).
- **20일 신고가·신저가 조회 기간**: `rs/compute_rs.py` 의 `week52()` 함수 `window` 인자(기본 20거래일).
