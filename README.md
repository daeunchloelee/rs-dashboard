...
- **하루 최대 상위 20개만 기록**: 신고가/신저가 종목이 많은 날엔 "오늘을 뺀 52주 구간의
  이전 최고가/최저가 대비 오늘 종가가 몇 % 위/아래인지"(갱신폭)가 큰 순으로 하루 최대
  `--top-n`(기본 20)개씩만 남깁니다. 개수는 `rs/compute_52w.py`의 `W52_TOP_N` 환경변수 또는
  워크플로에서 `--top-n` 인자로 바꿀 수 있습니다.
- **사유(자동 검색, 추정)**: 상위 20개로 추려진 종목마다 그 종목 이름으로 자동 검색한 최신
  관련 기사 1건을 "사유" 칸에 보여줍니다 — 한국 종목은 네이버 금융 뉴스, 미국 종목은
  Google 뉴스 RSS에서 가져옵니다. **사람이 실제 원인을 확인/검증한 게 아니라 추정**이라
  관련 없는 기사가 걸리거나 못 찾을 수 있습니다(그 종목 이름으로 뜨는 최신 기사일 뿐).
  공시(DART) 연동은 종목코드 → DART corp_code 매핑이 추가로 필요해서 아직 넣지 않았습니다
  — 원하면 나중에 추가할 수 있습니다. `--no-reason` 옵션으로 이 조회를 생략할 수 있습니다
  (뉴스 검색 실패해도 전체 실행은 절대 실패하지 않습니다 — 그 종목만 "관련 뉴스 없음"으로 표시).
- **데이터 크기 절약**: 스캔한 전 종목을 다 담지 않고, 위 상위 20개(신고가)+상위 20개(신저가)만
  `data/w52.json`에 저장합니다. `totalHigh52`/`totalLow52`(자르기 전 전체 건수)도 같이 저장해서
