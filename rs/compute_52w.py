...
# -*- coding: utf-8 -*-
"""
20일 신고가·신저가 스크리너 — 기존 rs/compute_rs.py(RS 대시보드)와는 완전히 독립적으로
동작하는 별도 스크립트다. compute_rs.py는 이 파일이 전혀 건드리지 않는다(import만 해서
KRX 캐시 폴백 로직 등 이미 검증된 코드를 재사용할 뿐).

RS(장기/단기 상대강도)는 계산하지 않는다 — 오직 종가 기준 20거래일(약 1개월, 원래
252거래일=52주로 했다가 너무 길다는 판단으로 줄임) 최고가/최저가만 본다. 그래서 장
마감 후 하루 한 번만 갱신해도 충분하고(실시간 필요 없음), 그 대신
...
그 안에서 "돌파/붕괴 강도"(오늘을 뺀 20일 구간의 이전 최고/최저 대비 오늘 종가가 몇 % 위/
...
def _process_one(args):
    ...
    if hi is None:
        return None
    # 돌파/붕괴 강도 — 오늘을 뺀 20일 구간의 이전 최고/최저 대비 오늘 종가가 몇 % 위/아래인지.
